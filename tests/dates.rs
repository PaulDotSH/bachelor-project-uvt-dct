...
async fn test_choices_open_date() {
    ...
    let start_diff = (db_start.timestamp() - start_date.timestamp()).abs();
    let end_diff = (db_end.timestamp() - end_date.timestamp()).abs();
    ...
    let new_start_date = chrono::Utc::now();
    let new_end_date = new_start_date + chrono::Duration::days(14);
    ...
    let updated_start_diff = (updated_dates.start_date.timestamp() - new_start_date.timestamp()).abs();
    let updated_end_diff = (updated_dates.end_date.timestamp() - new_end_date.timestamp()).abs();
